def is_spam_words(text, spam_words, space_around=False):
    ...
    for item in spam_words:
        if space_around:
            if text.find(' ' + item) != -1 or text.find(item + ' ') == 0 or text.find(item + '.') == 0:
                return True
        else:
            if text.find(item) != -1:
                return True
...
Слова у параметрі text можуть бути у довільному регістрі, а значить функція is_spam_words, при пошуку заборонених слів, регістру незалежна
і весь текст має зводитися до нижнього регістру.
...
Слово вважати, що стоїть окремо, якщо ліворуч від слова знаходиться символ пробілу або воно розташоване на початку тексту,
